...
class SetOtherShuttles extends Command
{
    protected $name = 'set:other_shuttles';

    private $shuttleType = Constants::SHUTTLE_TYPE_OTHER;

    public function handle()
    {
        Log::info("set:other_shuttles started.");
        $this->_setOtherShuttles(Constants::COMPANY_LOCATION_GENEL_MUDURLUK, "Pazarlama");
        $this->_setOtherShuttles(Constants::COMPANY_LOCATION_HOSDERE, "Hosdere");
        Log::info("set:other_shuttles finished.");

        return 'ok';
    }

    private function _setOtherShuttles($companyLocationId, $name)
    {
        //region Check File
        ...
        try {
            ...
            DB::rollBack();
            Log::error("set:other_shuttles transaction error. " . $e->getMessage());
        }
    }
}