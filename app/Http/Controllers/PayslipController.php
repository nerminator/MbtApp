...
class PayslipController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        if (!class_exists(SimpleXLSX::class)) {
            $fallbackParserPath = base_path('vendor/shuchkin/simplexlsx/src/SimpleXLSX.php');
            if (is_file($fallbackParserPath)) {
                require_once $fallbackParserPath;
            }
        }

        if (!class_exists(SimpleXLSX::class)) {
            Log::error('SimpleXLSX parser is missing for payslip import.');
            return back()->with('error', 'Excel parser is not available on server. Please contact support.');
        }
        ...
        try {
            ...
            Log::error('Payslip excel parse failed.', ['exception' => $e]);
            return back()->with('error', 'Excel file could not be parsed.');
        }

        if (!$xlsx) {
            Log::warning('Payslip excel parse returned false.', [
                'parse_error' => method_exists(SimpleXLSX::class, 'parseError') ? SimpleXLSX::parseError() : null,
            ]);
            return back()->with('error', 'Excel file could not be parsed.');
        }
