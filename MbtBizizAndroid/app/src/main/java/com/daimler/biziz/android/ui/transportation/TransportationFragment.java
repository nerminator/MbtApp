...
import com.daimler.biziz.android.databinding.FragmentTransportationBinding;
...
public class TransportationFragment extends BaseFragment {
    ...
    private void observe() {
        vmTransportation.getShuttleListResponse().observe(this, slr -> {
            //mock
            /*Gson gson = new Gson();
        ...
        vmTransportation.isSwitchButtonClicked.observe(this, (Boolean bool) -> {
            isTakeOff = !isTakeOff;
            if (!isTakeOff) {
        ...
        vmTransportation.toClicked.observe(this, aBoolean -> {
            if (isTakeOff) openSearchScreen(ArrivalField);
        });

        vmTransportation.fromClicked.observe(this, aBoolean -> {
            if (!isTakeOff) openSearchScreen(DepertureField);
        });

        vmTransportation.isLoading.observe(this, new Observer<Boolean>() {
            @Override
    ...
    private void setupViewPager(List<CompanyListEntity> shuttleCompanyList) {
        pagerAdapter.cleanFragments();
        binding.vpShuttle.setOffscreenPageLimit(shuttleCompanyList.size());
        for (int i = 0; i < shuttleCompanyList.size(); i++) {
            pagerAdapter.addFragment(ChildShuttleFragment.newInstance(shuttleCompanyList.get(i)), shuttleCompanyList.get(i).getName());
        }
